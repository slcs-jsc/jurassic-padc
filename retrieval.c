...
int main(
  ...
  double chisq, chisq_min, chisq_max, chisq_mean, cx, cy, sza_thresh, z[NP];
  ...
  cx = scan_ctl(argc, argv, "CX", -1, "8", NULL);
  cy = scan_ctl(argc, argv, "CY", -1, "2", NULL);
  ...
  while (fscanf(in, "%s", filename) != EOF) {
    ...
    /* Fill data gaps... */
    fill_gaps(ncd.l2_t, cx, cy);
    fill_gaps(ncd.l2_z, cx, cy);
	...
	/* Buffer results... */
	buffer_nc(&atm_i, &ncd, track, xtrack, np0, np1);
      }
...
void init_l2(
  ...
  for (ip = 0; ip < atm->np; ip++) {
    ...
    intpol_atm_1d(ctl, &atm_airs, 0, atm_airs.np, atm->z[ip], &p, &t, q, k);
